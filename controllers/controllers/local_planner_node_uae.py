...
class LocalPlannerNode(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            namespace='',
            parameters=[
                ('controller_type', 'uae_mpc'),
                ('config_file_path', 'path/to/your/experiment_config.yaml'),
                ('control_frequency', 20.0),
                ('map_frame', 'map'),
                ('base_link_frame', 'base_link'),
                ('use_external_sdf', False), # Use SDF from perception package
                ('seed', 2025)
            ]
        )
        ...
        if isinstance(self.controller, CUniformController) and self.controller.controller_type == 1:
            ...
            if self.use_external_sdf:
                self.get_logger().info("Controller will use SDF from perception package.")
    ...
    def sdf_callback(self, msg: OccupancyGrid):
        """Callback for SDF costmap from perception package."""
    ...
    def prepare_planner_input(self, local_goal_np):
        ...
        if costmap_np.shape != (expected_size, expected_size):
            self.get_logger().error(
                ...
                " Check perception node output and configuration file.",
                throttle_duration_sec=1.0
            )
            return None

        # 2. Process SDF (if required and available from perception)
        sdf_tensor = None
        if self.requires_sdf and self.use_external_sdf:
            if self.latest_sdf_msg is not None:
                ...
                self.get_logger().error("SDF required but not received from perception package", throttle_duration_sec=1.0)
    ...
    def process_sdf(self, msg: OccupancyGrid):
        ...
        try:
            ...
            # The perception node scales SDF by 10 and clips to int8 range
